...
export const SourceHostContext = React.createContext<SourceHostContextValue>(defaultSourceHostContextValue);

export const SourceHostContextProvider = ({children}: {children: ReactNode})=>{
    ...
    const [sourceHostName, setSourceHostName] = useState<string>(defaultSourceHostContextValue.sourceHostName);
    const [licenseAgreement, setLicenseAgreement] = useState<{[key:string]:boolean}>(defaultSourceHostContextValue.licenseAgreement);
    ...
    const agreeLicense = useCallback((sourceHostName: string) => {
        if(geoJsonServiceContext.geoJsonService?.catalog){
            const newLicenseAgreement = geoJsonServiceContext.geoJsonService?.catalog.properties.licenseAgreement || {};
            newLicenseAgreement[sourceHostName] = true;
            setLicenseAgreement(newLicenseAgreement);
            geoJsonServiceContext.geoJsonService?.catalog.update({properties: {licenseAgreement: newLicenseAgreement}});
        }
    }, [geoJsonServiceContext]);

    return <SourceHostContext.Provider value={{
        sourceHostName,
        setSourceHostName,
        licenseAgreement,
        agreeLicense
    }}>{children}</SourceHostContext.Provider>;
}
