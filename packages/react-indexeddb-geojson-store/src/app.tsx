...
import {BrowserRouter, Routes, Route, useNavigate, Outlet, useParams } from 'react-router';
import {Alert, Box, Button, Card, CardActionArea, CardContent, Dialog, DialogContent, Typography } from '@mui/material';
import { SelectStorageBundlePanel } from './components/SelectStorageBundlePanel';
...
import {DialogContextProvider, useDialogContext } from './hooks/useDialogContext';
import {SourceHostContextProvider, useSourceHostContext } from './hooks/useSourceHostContext';
import { SelectSourceHostPanel } from './components/SelectSourceHostPanel';
import {SourceHostNames, SourceHosts } from './types/SourceHosts';
import {StorageBundleContextProvider, useStorageBundleContext } from './hooks/useStorageBundleContext';
import {GeoJsonServiceContextProvider, useGeoJsonServiceContext } from './hooks/useGeoJsonServiceContext';
import { GeoJsonCountryMetadataContextProvider, useGeoJsonCountryMetadataContext } from './hooks/useGeoJsonCountryMetadata';
import { GeoJsonService } from './services/GeoJsonService';
import { SelectCountryAdminPanel } from './components/SelectCountryAdminPanel';
...
const Home = ()=> {
    ...
    return <Box><Button variant="contained" onClick={openDialog}>Open Dialog</Button></Box>
}
...
const DialogLuncherPage = ()=>{
    ...
    const handleNext = useCallback(() => {
        ...
        if (
            ...
            if(currentStepIndex === 0 && storageBundleContext.storageBundleName.length > 0 && ! storageBundleContext.storageBundleNames.includes(storageBundleContext.storageBundleName)){
                GeoJsonService.createInstance(storageBundleContext.storageBundleName).then((geoJsonService: GeoJsonService)=> {
                    next();
                });
            }else{
                next();
...
const SelectStorageBundlePage = ()=>{
    ...
    const onStorageBundleSelected = useCallback((storageBundleName: string) => {
        if(storageBundleName !== ""){
            dialogContext.setNextButtonEnabled(true);
            geojsonServiceContext.geoJsonService?.setDefaultDatabaseName(storageBundleName);
        }else{
    ...
    useEffect(() => {
        ...
        (async(): Promise<void> =>{
            storageBundleContext.setStorageBundleNames(await GeoJsonService.listDatabaseNames());
...
const SelectSourceHostPage = ()=>{
    const { sourceHostName: initialSourceHostName } = useParams();
    const dialogContext = useDialogContext();
    const sourceHostContext = useSourceHostContext();

    const handleSourceHostNameSelect = useCallback((newSourceHostName: string) => {
        if(newSourceHostName !== "" && SourceHosts[newSourceHostName] ){
            sourceHostContext.setSourceHostName(newSourceHostName);
            dialogContext.setNextButtonEnabled(sourceHostContext.licenseAgreement[newSourceHostName]);
        }else{
            dialogContext.setNextButtonEnabled(false);
        }
    }, []);
    ...
    useEffect(() => {
        const newSourceHostName = initialSourceHostName? initialSourceHostName : sourceHostContext.sourceHostName ? sourceHostContext.sourceHostName : SourceHostNames.GADM;
        handleSourceHostNameSelect(newSourceHostName);
        dialogContext.setNextButtonEnabled(sourceHostContext.licenseAgreement[sourceHostContext.sourceHostName]);
    }, [initialSourceHostName, sourceHostContext.sourceHostName, sourceHostContext.licenseAgreement]);
    ...
    return (
        <SelectSourceHostPanel agreeLicense={handleAgreeLicense} />
    );
}

const SelectCountryAdminPage = ()=>{
    ...
    useEffect(()=>{
        setSelectCheckboxMatrix(countryMetadataContext.countryMetadataArray.map((countryMetadata: CountryMetadata) => new Array<boolean>(countryMetadata.maxAdminLevel+1)));
    }, [countryMetadataContext]);

    if(storageBundleName === undefined || sourceHostName === undefined || countryMetadataContext.countryMetadataArray.length === 0){
        return "";
    }

    const downloadedMatrix = Array<Array<boolean>>();

    return <SelectCountryAdminPanel
        countryMetadataArray={countryMetadataContext.countryMetadataArray}
        maxAdminLevel={countryMetadataContext.maxAdminLevel}
...
const DownloadCountryAdminPage = ()=>{
    const { storageBundleName, sourceHostName } = useParams();
    const dialogContext = useDialogContext();
        ...
        <BrowserRouter>
            <Routes>
                <Route path={`/${BASE}`} element={<Layout/>}>
                    ...
                    <Route path="geojson-store" element={(
                        ...
                        <Route index element={<SelectStorageBundlePage />}/>
                        <Route path=":storageBundleName" element={(
                            <SourceHostContextProvider>
                                <Outlet/>
                            </SourceHostContextProvider>)
                        }>
                            <Route index element={(
                              <SelectSourceHostPage/>
                            )}/>
                            <Route path="s/:sourceHostName" element={(
                              <GeoJsonCountryMetadataContextProvider>
                                  <Outlet/>
                              </GeoJsonCountryMetadataContextProvider>
                            )}>
                                <Route index element={(<SelectCountryAdminPage />)}/>
                                <Route path="download" element={(<DownloadCountryAdminPage />)}/>
                            </Route>
                        </Route>
                    </Route>
                </Route>
